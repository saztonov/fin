...
describe('Ставка НДС акта по концу периода', () => {
  ...
  it('ставка меняет нетто акта на величину, заметную в деньгах', () => {
    // Документ целиком: грид выделяет НДС построчно и складывает построчное, так
    // что на реальном акте из 26 строк (КС-2 №1, Садовническая 69) получается
    // 55 175 779,16 против 56 095 375,49 — разница 919 596,33 ₽. Здесь та же
    // разница на итоге документа: масштаб ошибки от способа суммирования не зависит.
    const gross = '67314450.60';
    const atEnd = netFromGrossRate(gross, periodVatRate('2025-12-01', '2026-02-18'));
    const atStart = netFromGrossRate(gross, periodVatRate('2025-12-01', null));
    expect(atEnd).toBe('55175779.18');
    expect(atStart).toBe('56095375.50');
    expect(Number(atStart) - Number(atEnd)).toBeCloseTo(919596.32, 2);
  });
});
