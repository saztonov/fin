...
- НДС выделяется из суммы с НДС по ставке на дату периода: 20% до 31.12.2025,
  22% с 01.01.2026 (таблица `vat_rates`). Для договорных строк дата берётся из ДС,
  а без ДС — из договора. Договор с `vat_mode = 'net'` ведётся без НДС — выделение
  не выполняется. Только при отображении/экспорте.
